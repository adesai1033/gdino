...
def yolo_to_xyxy(yolo_bbox, is_5):
    cx, cy, w, h = yolo_bbox[0], yolo_bbox[1], yolo_bbox[2], yolo_bbox[3]
    topleft_x = cx - w/2
    topleft_y = cy - h/2
    btmright_x = cx + w/2
    btmright_y = cy + h/2
    if is_5:
        return [topleft_x, topleft_y, btmright_x, btmright_y, yolo_bbox[4]]
    else:
...
def gtfile_to_list(textfile):
    ...
    for line in lines:
        box = line.split(' ')
        floatbox = [float(x) for x in box[1:]]
        boxes_lst.append(yolo_to_xyxy(floatbox, False))
    return boxes_lst
def mfile_to_list(textfile):
    ...
    for line in lines:
        ...
        xyxy = list(tensorbox)
        boxes_lst.append(yolo_to_xyxy(xyxy, False))
    return boxes_lst
