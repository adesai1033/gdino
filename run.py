...
def overlay_and_write(image_source, boxes_after_nms, logits_after_nms, phrases, filenum, path):
    annotated_frame = annotate(image_source=image_source, boxes=boxes_after_nms, logits=logits_after_nms, phrases=phrases)
    cv2.imwrite(os.path.join(path, f'annotated{filenum}.png'), annotated_frame)

def generate_textfile(boxes_after_nms, logits_after_nms, filenum, path):
    
    annotation_file = os.path.join(path, f"annotated{filenum}.txt")
    # Write the contents of boxes_after_nms and logits_after_nms to the file
    with open(annotation_file, "w") as f:
        f.write('Boxes after NMS:\n')
...
def main():
    ...
    INPUT_DIR = 'input_frames3'
    TEXT_PROMPT = "person from above."
    BOX_THRESHOLD = 0.2
    TEXT_THRESHOLD = 0.2
    NMS_THRESHOLD = 0.7
    OUTPUT_DIRECTORY1 = 'dino_output_images3'
    OUTPUT_DIRECTORY2 = 'dino_output_text3'
    ...
    filenum = 0
    for file in os.listdir(INPUT_DIR): #fixlater
        #read file
        ...
        overlay_and_write(image_source, boxes_after_nms, logits_after_nms, phrases, filenum, PARENT_DIRECTORY + OUTPUT_DIRECTORY1)
        generate_textfile(boxes_after_nms, logits_after_nms, filenum, PARENT_DIRECTORY + OUTPUT_DIRECTORY2)
