from groundingdino.util.inference import load_model, load_image, predict, annotate
import os
from torchvision.ops import box_convert
import cv2
from precision_recall import gtfile_to_list
import torch
import numpy as np

def overlay_boxes(image, gt_boxes, filenum, path):
    for gt_box in gt_boxes:
        height, width, channels = image.shape
        #gt_box_tensor = torch.Tensor(gt_box)
        #xyxy = list(box_convert(boxes=gt_box_tensor, in_fmt="cxcywh", out_fmt="xyxy").numpy())
        xyxy = gt_box.copy()
        start_x, start_y, end_x, end_y = int(xyxy[0] * width), int(xyxy[1] * height), int(xyxy[2] * width), int(xyxy[3] * height)
        #start_x, start_y, end_x, end_y = int(xyxy[0] ), int(xyxy[1]  ), int(xyxy[2] ), int(xyxy[3])
        cv2.rectangle(image, (start_x, start_y), (end_x, end_y), (255, 0, 0), 1)
    
        cv2.imwrite(os.path.join(path, f'bounding_box_image{filenum}.png'), image)
def main():
    
    IMAGE_DIR = 'accuracy_img/'
    GT_TXT_DIR = '/mnt/bpd_architecture/abdesa/ground_truth_text/'
    GT_IMG =  '/mnt/bpd_architecture/abdesa/ground_truth_img_test/'
    PARENT_DIRECTORY = '/mnt/bpd_architecture/abdesa/'
    os.mkdir(os.path.join(PARENT_DIRECTORY, GT_IMG))

    filenum = 0
    for txt, img in zip(os.listdir(GT_TXT_DIR), os.listdir(IMAGE_DIR)): #fixlater
        #read file
        txt_path = os.path.join(GT_TXT_DIR, txt)
        image_path = os.path.join(IMAGE_DIR, img)
        
        image = cv2.imread(image_path)
        gtboxes_lst = gtfile_to_list(txt_path)
        overlay_boxes(image, gtboxes_lst, filenum,GT_IMG)

        print("filenum:", filenum)
        filenum += 1

if __name__ == '__main__':
    main()